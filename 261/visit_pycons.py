...
# Your code #
def update_pycons_lat_lon(pycons: List[PyCon]) -> List[PyCon]:
    """
    ...
    """
    with open(nominatim_responses, encoding="utf-8") as fp:
        responses = json.load(fp)
    for pycon in pycons:
        url = f"https://nominatim.openstreetmap.org/search?q={pycon.city},{pycon.country}&format=json&accept-language=en"
        pycon.lon = float(responses[url][0]["lon"])
        pycon.lat = float(responses[url][0]["lat"])
    return pycons
...
def create_travel_plan(pycons: List[PyCon]) -> List[Trip]:
    ...
    """
    trips = []
    for src, dst in zip(pycons[0:], pycons[1:]):
        trip = Trip(src, dst, _km_distance(src, dst))
        trips.append(trip)
    return trips
...
if __name__ == "__main__":
    pycons = update_pycons_lat_lon(_get_pycons())
    trips = create_travel_plan(pycons)