...
def parse_social_platforms_string():
    ...
    platforms = {
        "Twitter": Validator(range(1, 15), re.compile(r"[a-zA-Z0-9_]+")),
        "Facebook": Validator(range(5, 50), re.compile(r"[a-zA-Z0-9.]+")),
        "Reddit": Validator(range(3, 20), re.compile(r"[a-zA-Z0-9_-]+")),
    }
    return platforms
...
def validate_username(platform, username):
    ...
    try:
        ...
        raise ValueError("Platform doesn't exist.")
    else:
        valid_length = len(username) in validator.range
        valid_chars = re.fullmatch(validator.regex, username)
        return True if valid_length and valid_chars else False


if __name__ == "__main__":
    pfs = social_platforms.split("\n\n\n")
    print(pfs)